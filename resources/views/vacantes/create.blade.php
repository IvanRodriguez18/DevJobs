...
@section('content')
<h1 class="text-center text-2xl text-gray-600 mt-5">Registra Una Nueva Vacante</h1>
<form action="{{ route('vacantes.store') }}" method="POST" class="w-full max-w-lg mx-auto mt-5 bg-white rounded p-3 shadow-md mb-3" enctype="multipart/form-data" novalidate>
    @csrf
    <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
            ...
            <input type="text" name="vacante" class="w-full block bg-gray-200 text-gray-700 appearance-none rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white focus:border-gray-500 leading-tight @error('vacante') is-invalid @enderror" id="vacante" placeholder="Ejemplo: FullStack" value="{{ old('vacante') }}">
            @error('vacante')
        ...
        <div class="w-full px-3">
            ...
            <select name="experiencia" class="block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white focus:border-gray-500 leading-tight" id="experiencia">
                ...
                @foreach($experiencias as $experiencia)
                    <option {{ old('experiencia') == $experiencia->id ? 'selected' : '' }} value="{{ $experiencia->id }}">{{ $experiencia->experiencia }}</option>
                @endforeach
            </select>
            @error('experiencia')
                <span class="text-xs text-red-700 p-2 w-full mt-5 text-left" role="alert">
        ...
        <div class="w-full px-3 md:w-1/2 md:mb-0 mb-6">
            ...
            <select name="salario" class="block w-full bg-gray-200 text-gray-700 rounded py-3 px-4 mb-3 focus:outline-none focus:bg-white focus:border-gray-500 leading-tight" id="salario">
                ...
                @foreach($salarios as $salario)
                    <option {{ old('salario') == $salario->id ? 'selected' : '' }} value="{{ $salario->id }}">{{ $salario->salario }}</option>
                @endforeach
            </select>
            @error('salario')
                <span class="text-xs text-red-700 p-2 w-full mt-5 text-left" role="alert">
    ...
    <div class="flex flex-wrap -mx-3 mb-6">
        <div class="w-full px-3">
            ...
            @endphp
            <lista-skills :skills="{{ json_encode($skills) }}" :allskills="{{ json_encode(old('skill')) }}"></lista-skills>
            @error('skill')
                <span class="text-xs text-red-700 p-2 w-full mt-5 text-left" role="alert">
                    <strong>{{ $message }}</strong>
                </span>
